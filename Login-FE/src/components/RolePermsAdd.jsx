...
import { useRolePermsAddPost } from "../hooks/data/useRolePermsAddPost";
...
// Main component
function RolePermsAdd(props) {
  console.log("RUNNING ROLEPERMSADD COMPONENT");
  // state key data element and row
  const { role, getRolePerms, mode, setOpen, setModeResult } = props;
  const [rowSelection, setRowSelection] = useState({});
  const [permsToAdd, setPermsToAdd] = useState([]);
  ...
  const rolePermsAdd = useRolePermsAddPost();
  ...
  const handleSubmit = async () => {
    ...
    Object.keys(rowSelection).map((perm) => {
      addedPerms.push({ roleid: role.id, permid: parseInt(perm) });
    });
    ...
    try {
      const resp = await rolePermsAdd.mutateAsync(addedPerms);
      console.log("userRolesAdd Resp", resp);
      console.log("ADDED ROLES", addedPerms);
      setOpen(false);
      setModeResult(true);
    } catch (error) {
      setReqError(error?.response?.status);
      setFormError(error?.message);
      console.log("ADDED PERMS ERROR");
      // setReqError(error?.response?.status);
  ...
  useEffect(() => {
    const test = getPerms?.data?.data.filter((perm) => {
      const test2 = !getRolePerms?.data?.data.find((roleperm) => {
        return perm.id === roleperm?.permid;
      });
      return test2;
    });
    setPermsToAdd(test);
  ...
  useEffect(() => {
    ...
    if (getPerms?.error?.response?.status === 403) {
      ...
      <>
        <Stack direction="row">
          ...
          >
            ...
            <Box className="tableBox" sx={{ height: "calc(100% - 1.75rem)" }}>
              <MaterialReactTable
                // Primary props
                initialState={{
                ...
                // muiTopToolbarProps={{ ref: barRef }}
                muiTableContainerProps={{
                  // height h(view port) -h(appbar) -h(title)- h(table tools) -h(buffer))
                  sx: {
                    // height: "calc( 75vh - 1.75rem - 70px - 100px)",

                    height: "calc(100% - 1.75rem - 44px)",
                    maxHeight: "none",
                    overflow: "auto",
                    // minHeight: "200px",
                  },
